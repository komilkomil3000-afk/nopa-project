...
async function main() {
  ...
  async function upsertUser(phone: string, updateData: any, createData: any) {
  ...
  // 1. Admins
  const alireza = await upsertUser(
    '09196657042',
    { name: 'علیرضا خوشمنظر', role: 'admin', zarikBalance: 0, levelFrame: 1 },
    { phoneNumber: '09196657042', name: 'علیرضا خوشمنظر', role: 'admin', passwordHash, zarikBalance: 0, levelFrame: 1, identityVerified: true }
  );

  // 2. Mentors
  const mentorKoveyti = await upsertUser(
    mentorKoveytiPhone,
    { name: 'محمد کویتی', role: 'mentor', zarikBalance: 0, levelFrame: 1 },
    { phoneNumber: mentorKoveytiPhone, name: 'محمد کویتی', role: 'mentor', passwordHash, zarikBalance: 0, levelFrame: 1, identityVerified: true }
  );

  const mentorJalali = await upsertUser(
    mentorJalaliPhone,
    { name: 'رضا جلالی', role: 'mentor', zarikBalance: 0, levelFrame: 1 },
    { phoneNumber: mentorJalaliPhone, name: 'رضا جلالی', role: 'mentor', passwordHash, zarikBalance: 0, levelFrame: 1, identityVerified: true }
  );
  ...
  for (const st of students) {
    await upsertUser(
      st.phone,
      { name: st.name, role: 'student', zarikBalance: 0, levelFrame: 1, caravanId: st.caravanId },
      { phoneNumber: st.phone, name: st.name, role: 'student', passwordHash, zarikBalance: 0, levelFrame: 1, caravanId: st.caravanId, identityVerified: true }
    );
  }
