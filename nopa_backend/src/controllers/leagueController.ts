...
export async function getCaravansLeaderboardDetailed(req: Request, res: Response) {
  try {
    ...
    if (exportAs === 'csv') {
      ...
      filtered.forEach((c) => {
        let rStr = '-';
        if (c.totalScore > 0) {
          if (prevSc === null) currentR = 1;
          else if (c.totalScore < prevSc) currentR++;
...
export async function getIndividualsLeaderboard(req: Request, res: Response) {
  try {
    const role = (req.query.role as string) || 'all'; // 'all' | 'student' | 'mentor'
    ...
    if (exportAs === 'csv') {
      ...
      filtered.forEach((u) => {
        ...
        if (u.totalScore > 0) {
          if (prevSc === null) currentR = 1;
          else if (u.totalScore < prevSc) currentR++;
          rStr = String(currentR);
          prevSc = u.totalScore;
        }
