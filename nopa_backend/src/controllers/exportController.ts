...
export const exportData = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (type === 'users') {
      ...
      data = filtered.map((c) => {
        ...
        if (c.totalScore > 0) {
          ...
          else if (c.totalScore < prevScore) currentRank++;
      ...
      data = filtered.map((u) => {
        ...
        if (u.totalScore > 0) {
          if (prevScore === null) currentRank = 1;
          else if (u.totalScore < prevScore) currentRank++;
          rankStr = String(currentRank);
    ...
    else if (type === 'league_caravans' || type === 'caravans_league') reportTitle = 'جدول رتبه‌بندی لیگ کاروان‌ها (جام زرین)';
    else if (type === 'league_individuals' || type === 'individuals_league') reportTitle = 'جدول رتبه‌بندی انفرادی اعضا و دانش‌آموزان (جام زرین)';
