...
async function main() {
    ...
    const sheet04 = XLSX.utils.sheet_to_json(wb2.Sheets['04'], { header: 1 });
    const mentorIdMap = {}; // sheet ID -> db ID
    ...
    const sheet02 = XLSX.utils.sheet_to_json(wb2.Sheets['02'], { header: 1 });
    // To link correctly, we see Sheet 01 has mapping between group and mentor.
    const sheet01 = XLSX.utils.sheet_to_json(wb2.Sheets['01'], { header: 1 });
    const groupToMentorSheetId = {};
    ...
    // Assets (Sheet 06)
    const sheet06 = XLSX.utils.sheet_to_json(wb2.Sheets['06'], { header: 1 });
    ...
    const sheet03 = XLSX.utils.sheet_to_json(wb2.Sheets['03 '], { header: 1 });
    ...
    if (sheet03Name) {
        const s03 = XLSX.utils.sheet_to_json(wb2.Sheets[sheet03Name], { header: 1 });
        // Group by station
    ...
    if (sheet05Name) {
        const s05 = XLSX.utils.sheet_to_json(wb2.Sheets[sheet05Name], { header: 1 });
