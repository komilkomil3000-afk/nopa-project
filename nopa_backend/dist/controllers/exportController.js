...
const exportData = async (req, res) => {
    ...
    try {
        ...
        if (type === 'users') {
            ...
            data = mentors.map((m, idx) => {
                ...
                if (m.mentorLevel === 3)
                    ...
                    levelStr = 'سطح ۲ (پیشرفته)';
                let statusStr = 'فعال';
                if (m.accountStatus === 'SUSPENDED' || m.blocked || m.isDeleted)
                    statusStr = 'مسدود / تعلیق';
                else if (m.accountStatus === 'PENDING_VERIFICATION')
                    statusStr = 'در انتظار بررسی';
                return [
                    idx + 1,
        ...
        if (type === 'mentors')
            reportTitle = 'گزارش جامع شناسنامه و مشخصات راهبران و اساتید آموزشی';
        else if (type === 'caravans')
            reportTitle = 'گزارش وضعیت و پیشرفت کاروان‌های آموزشی';
        else if (type === 'users')
            reportTitle = 'فهرست کاربران و دانش‌آموزان سامانه';
        else if (type === 'ledger')
        ...
        if (format === 'csv') {
            ...
            sheet.columns.forEach((column, cIdx) => {
                let maxLen = 14;
                if (headers[cIdx])
                    maxLen = Math.max(maxLen, headers[cIdx].length * 2);
                column.width = maxLen;
            });
