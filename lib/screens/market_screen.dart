...
class _MarketScreenState extends State<MarketScreen> {
  ...
  Widget _buildProfileUpgradeSection(BuildContext context, AppRepository repository) {
    final userRole = repository.currentUser.role;
    final int zarikBalance = repository.currentUser.zarikBalance;
    
    // Map levelFrame to ProfileLevel
    ProfileLevel currentLvl = ProfileLevel.newcomer;
    final frame = repository.currentUser.levelFrame;
    if (frame == 2) currentLvl = ProfileLevel.general;
    if (frame == 3) currentLvl = ProfileLevel.bronze;
    if (frame == 4) currentLvl = ProfileLevel.silver;
    if (frame == 5) currentLvl = ProfileLevel.golden;

    final List<Map<String, dynamic>> levelsShop = [
      {
        'level': ProfileLevel.general,
        'name': 'سطح عمومی',
        'price': 500,
        'color': Colors.blue,
      },
      {
        'level': ProfileLevel.bronze,
        'name': 'سطح برنزی',
        'price': 1500,
        'color': const Color(0xFFCD7F32),
      },
      {
        'level': ProfileLevel.silver,
        'name': 'سطح نقره‌ای',
        'price': 3000,
        'color': const Color(0xFFC0C0C0),
      },
      {
        'level': ProfileLevel.golden,
        'name': 'سطح طلایی',
        'price': 5000,
    ...
    return Column(
      ...
      children: [
        ...
        ListView.builder(
          ...
          itemCount: levelsShop.length,
          itemBuilder: (context, index) {
            final item = levelsShop[index];
            final ProfileLevel lvl = item['level'];
            final bool isOwned = userRole == UserRole.member && currentLvl == lvl;
                ...
                children: [
                    ...
                    onPressed: isOwned
                        ...
                        : () async {
                            if (userRole == UserRole.member) {
                              if (zarikBalance >= item['price']) {
                                final api = HttpApiService();
                                final success = await api.buyZarikPackage(-(item['price'] as int)); // Deduct
                                if (success) {
                                  await repository.refreshUser(); // Update state from backend
                                  if (context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text('قاب ${item['name']} با موفقیت خریداری و درخواست اعمال ارسال شد! 🎉', style: const TextStyle(fontFamily: 'Vazirmatn')),
                                        backgroundColor: const Color(0xFF10B981),
                                      ),
                                    );
                                  }
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('امتیاز زریک شما برای خرید این قاب کافی نیست ❌', style: TextStyle(fontFamily: 'Vazirmatn')),
                                    backgroundColor: Colors.redAccent,
                                  ),
                                );
                              }
                            } else {
                              _showMentorTicketDialog(context, item['name']);
                            }
                          },
                    style: ElevatedButton.styleFrom(
                      backgroundColor: isOwned ? Colors.grey : const Color(0xFF8B5CF6),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                  ...
                  Column(
                    ...
                    children: [
                      Text(
                        item['name'],
                        style: TextStyle(color: item['color'], fontSize: 14, fontWeight: FontWeight.bold, fontFamily: 'Vazirmatn'),
                      ),
                      const SizedBox(height: 4),
                      Text(
                        userRole == UserRole.member
                            ? 'هزینه: ${item['price']} زریک'
                            : 'نیازمند تیکت و تایید مدیریت',
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: GlobalState.getLevelGlow(lvl),
                    ),
                    child: ClipOval(
                      child: Image.network(
  ...
  void _showMentorTicketDialog(BuildContext context, String levelName) {
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'مثلاً: با سلام، درخواست تایید شایستگی جهت دریافت قاب ارتقا یافته را دارم...',
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('تیکت درخواست ارتقا به $levelName با موفقیت به مدیریت ارسال شد.', style: const TextStyle(fontFamily: 'Vazirmatn')),
                        backgroundColor: const Color(0xFF10B981),
                      ),
                    );
                  },
                  ...
                  child: const Text('ارسال تیکت رسمی', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontFamily: 'Vazirmatn')),
                ),
              )
            ],
          ),
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Section 4: Buy & Upgrade Profile Frames
                _buildProfileUpgradeSection(context, repository),
